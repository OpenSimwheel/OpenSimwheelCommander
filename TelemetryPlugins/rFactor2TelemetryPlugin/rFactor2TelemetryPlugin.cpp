#include "rFactor2TelemetryPlugin.h"
...
rFactor2TelemetryPlugin::rFactor2TelemetryPlugin() {
...
QWidget* rFactor2TelemetryPlugin::GetSettingsWidget() {
    QWidget* widget = new SettingsGroupBox(pluginSettings);
    return widget;
}

void rFactor2TelemetryPlugin::Startup() {
    if (!isInitialized)
        Initialize();
}

bool rFactor2TelemetryPlugin::Initialize() {
    if (!hMapFile) {
...
void rFactor2TelemetryPlugin::Shutdown() {
    if (pSharedMem)
...
bool rFactor2TelemetryPlugin::IsNewDataAvailable()
{
...
bool rFactor2TelemetryPlugin::WaitForNewData(int timeout)
{
    if (isInitialized || Initialize())
    {
...
TelemetryInfo rFactor2TelemetryPlugin::GetNewData()
{
...
TelemetryFeedback rFactor2TelemetryPlugin::Update()
{
