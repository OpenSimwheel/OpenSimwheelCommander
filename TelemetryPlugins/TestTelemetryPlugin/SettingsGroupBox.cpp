...
//    ui->checkBox_useHardstops->setChecked(this->settings->GetUseHardstops());
    ...
    ui->max_steeringArmForce->setValue(this->settings->GetMaxSteeringArmForce());
    ui->checkBox->setChecked(this->settings->GetEnable());
    ui->checkBox_invert->setChecked(this->settings->GetIsInverted());
}
...
void SettingsGroupBox::on_max_steeringArmForce_valueChanged(int arg1)
{
    this->settings->SetMaxSteeringArmForce(arg1);
    this->settings->Save(category);
}

void SettingsGroupBox::on_checkBox_invert_toggled(bool checked)
{
    this->settings->SetIsInverted(checked);
