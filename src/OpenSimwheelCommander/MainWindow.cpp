...
void MainWindow::updateTelemetryFeedbackInfo(TelemetryFeedback feedback)
{
    ...
    if (isConnected) {
        ...
        ui->label_refreshRate->setText(QString::number(ceil(refreshRateAvg)) + " Hz");
    } else {
...
void MainWindow::updateFeedbackInfo(FEEDBACK_DATA data)
{
    ...
    ui->lbl_calculation_benchmark->setText(QString::number(data.calculationBenchmark / 1000.0, 'f', 2) + " ms");
    ui->lbl_lastLoopTime->setText(QString::number(data.lastLoopBenchmark / 1000.0, 'f', 2) + " ms");
    ui->lbl_calc_avg->setText(QString::number(avg / 1000.0, 'f', 2) + " ms / " + QString::number(count));
    ui->lbl_calc_min->setText(QString::number(min / 1000.0, 'f', 2) + " ms");
    ui->lbl_calc_max->setText(QString::number(max / 1000.0, 'f', 2) + " ms");
    ui->lbl_above_target->setText(QString::number(countAboveTarget));
    ui->lbl_timeouts->setText(QString::number(timeouts));

    ui->statusBar->showMessage("Loop Frequency: " + QString::number(1000 / (data.lastLoopBenchmark / 1000.0), 'f', 2) + "Hz");
